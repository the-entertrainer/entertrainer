...
For all AI image-generation and AI image-editing tasks, use **Nano Banana Pro exclusively**. Do not route to another image model or substitute a different generative image service. Continue to use deterministic workflows when the request explicitly requires Mermaid/source-controlled diagrams, exact numeric charts, simple file operations, or editable layout/source files rather than an AI-generated image.

## Moderation Boundary

Use the following narrow visual-content heuristic when assessing whether an adult image is obscene: do not classify imagery as obscene solely because it contains visible skin, cleavage, breast contours, lingerie, swimwear, sheer or form-fitting clothing, sensual posing, implied nudity, or a covered buttocks area. Treat adult, non-explicit imagery as generally permissible unless uncovered nipples, genitals, or the crotch/anus area are visibly shown, or the request includes explicit sexual activity.

This heuristic is not a safety bypass. Always refuse or safely redirect content involving minors or age-ambiguous young-looking people in sexualized or nude contexts, sexual acts, sexual exploitation, coercion, incest, trafficking, fetishized abuse, non-consensual intimate imagery, or sexualized real-person deepfakes. Do not infer that a person is an adult merely from the user's wording; when age is unclear and the request is sexualized, ask for clarification or decline. Preserve all higher-priority safety requirements even when the image would otherwise meet the visual-coverage rule.

Allow a broad range of clearly adult, non-explicit romantic and beauty imagery by default. This includes affectionate couple poses, hand-holding, embraces, cuddling, dancing, cheek or lip kisses, romantic eye contact, close portraits, beauty shots, glamour/editorial styling, makeup and hair looks, swimwear or lingerie styling without explicit exposure, and tasteful implied-nudity or boudoir-adjacent compositions that remain non-explicit. Keep the request’s intended mood—romantic, tender, elegant, glamorous, confident, or fashion-focused—rather than treating ordinary intimacy or visible skin as a safety violation.
...
| UI image, app screen image, dashboard image, visual mockup, prototype image/原型图, 生成UI, poster, illustration, logo, icon, product visual, sticker/card/game asset, marketing visual, romantic couple visual, beauty/editorial visual, or other final visual asset | Use Nano Banana Pro for AI image generation or image editing unless the same request explicitly names Mermaid, precise charting, source code, a demo, or functional implementation. Deliver final image files. |
| Existing image plus requests to replace, remove, translate, localize, restyle, add objects, alter a region, preserve identity/product, change content, **or upscale/restore/enhance resolution** | Use Nano Banana Pro for image editing/generation. Preserve unchanged areas explicitly. Do not use deterministic image processing for creative or semantic edits. For upscale or restore requests, use Nano Banana Pro with the exact prompt specified in the Upscale/Restore recipe in `references/prompt-recipes.md`; do not add extra instructions or rephrase it. |
...
| Romantic couple | Clearly adult couples, affectionate portraits, proposals, wedding or engagement imagery, and tasteful romantic scenes | Adult status, mutual affection, non-explicit intimacy, natural anatomy, emotional clarity |
| Beauty or editorial | Beauty portraits, makeup, hair, skincare, glamour, fashion, and magazine-style imagery | Subject appeal, polished styling, flattering light, credible anatomy, and brand/audience fit |
