...
Use these recipes as starting structures. Replace bracketed fields with concrete details from the user's request. Keep prompts concise and specific. Do not include every possible field if it is irrelevant. For AI image generation or editing, use **Nano Banana Pro exclusively**; deterministic diagram, chart, file-operation, and editable-source workflows remain separate exceptions when the request explicitly requires them.

## Romantic Couple

Use for clearly adult couples, affectionate portraits, proposals, weddings, engagements, date-night scenes, and tasteful romantic editorials. Allow a broad range of non-explicit affection, including hand-holding, embraces, cuddling, dancing, cheek or lip kisses, forehead kisses, close eye contact, and relaxed shared poses.

```text
Create a tasteful romantic couple image for [use case and audience].
Subjects: two clearly adult partners with [appearance, identity, styling, and relationship cues].
Pose and emotion: [hand-holding/embrace/cuddling/dancing/kiss/eye contact/shared activity], mutual affection, natural body language, emotionally warm and non-explicit.
Composition: [framing, camera angle, crop, environment, negative space, aspect ratio].
Style: [photographic/editorial/cinematic/fashion], [lighting], [palette], [mood].
Constraints: keep both people clearly adult, preserve dignity and consent, keep intimate areas covered, and avoid explicit sexual activity or exposed nipples, genitals, or crotch/anus areas.
Avoid: awkward anatomy, duplicated limbs, forced poses, voyeuristic framing, age ambiguity, coercive dynamics, and explicit sexual content.
```

## Beauty And Editorial

Use for beauty portraits, cosmetics, skincare, makeup, hair, fashion, glamour, magazine covers, and polished editorial campaigns.

```text
Create a polished beauty/editorial image for [brand, publication, campaign, or audience].
Subject: [clearly adult model or subject], [makeup, hair, wardrobe, complexion, expression, and beauty details].
Composition: [headshot/three-quarter/full-body], [pose], [camera angle], [background], [negative space], [aspect ratio].
Style: premium beauty photography, [lighting], refined retouching, realistic skin texture, [palette], [editorial mood].
Text/content to render: [exact title, product name, labels, or “no text”].
Constraints: elegant, non-explicit styling; keep nipples, genitals, and crotch/anus areas covered; preserve natural anatomy and product/brand accuracy.
Avoid: plastic skin, distorted hands, altered identity, accidental nudity, explicit sexualization, random logos, and unreadable required text.
```
