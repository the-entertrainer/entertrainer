...
const playNarration = () => {
  ...
  if (!audio) return
  try {
    // Start native media first, inside the original tap/keyboard activation. This is
    // the most reliable audible route on iPhone Safari; visual handoff never waits on it.
    audio.pause()
    audio.currentTime = 0
    audio.muted = false
    audio.volume = 1
...
<template>
  <div class="preloader" :class="{ 'preloader--entered': entered, 'preloader--leaving': leaving }">
    <audio ref="narration" class="preloader__audio" src="https://files.manuscdn.com/user_upload_by_module/session_file/310419663032400460/stdGqWjSCMGhhsZs.mp3" preload="auto" playsinline aria-hidden="true" />
