...
const playNarration = (fromGesture = false) => {
  const audio = narration.value
  if (!audio) return
  ...
  try {
    ...
    if (!context) return
    audio.volume = 0.94
    if (fromGesture) audio.currentTime = 0
    // Keep both calls in the direct click stack. This is the condition iOS Safari
    // requires before it will permit audible media and Web Audio to start.
    void context.resume().catch(() => undefined)
    const attempt = audio.play()
    void attempt.then(() => { needsSoundGesture.value = false }).catch(() => {
      if (!completed) needsSoundGesture.value = true
    })
  } catch {
    if (!completed) needsSoundGesture.value = true
  }
...
const enableSound = () => {
  ...
  playNarration(true)
  // A visitor who explicitly requests the audio gets time to hear the complete line.
  finishTimer = window.setTimeout(finish, 4400)
}
...
onMounted(() => {
  ...
  finishTimer = window.setTimeout(finish, reduced ? 140 : 4400)
})
onBeforeUnmount(() => {
  if (finishTimer) window.clearTimeout(finishTimer)
...
<template>
  <div class="preloader" :class="{ 'preloader--leaving': leaving }">
