...
useSeoMeta({
  ...
  description: 'A short, visual, beginner course tracing AI from early ideas to responsible modern use. No magic fog, just the machinery underneath.',
  ogTitle: 'From No AI to Know AI · Entertrainer',
  ogDescription: 'A short, visual beginner course from AI’s early ideas to responsible modern use. No magic fog, just the machinery underneath.',
...
const screens: Screen[] = [
  { lesson: 'Welcome', label: 'Start', kind: 'cover', title: 'From No AI to Know AI', body: ['Artificial intelligence did not begin with ChatGPT. This course follows the longer route from early questions about machine intelligence to prediction, modern systems, and responsible use.'] },
  { lesson: 'Welcome', label: 'Objectives', kind: 'objectives', title: 'What you should be able to do by the end', body: ['By the end of this module, you will be able to place modern AI in a longer history, explain how a prediction system works in simple terms, recognise different types of AI input, and describe one responsible-use check.'] },
  ...
  { lesson: 'Lesson 1 · Before ChatGPT', label: 'Work bench', kind: 'media', title: 'AI developed alongside physical computers', body: ['Before modern software ran in a browser, researchers and operators worked with physical computing systems. The field did not suddenly appear with one new product.'], media: 'earlyBench', caption: 'Notice the route from cards and relays toward modern hardware: the questions changed over time, but the work stayed tangible.' },
  ...
  { lesson: 'Lesson 2 · What AI is', label: 'System example', kind: 'media', title: 'Keep the task, output, and human check visible', body: ['Start with a bounded task. Name the information the system receives. Treat its result as a candidate output. Then name the person or evidence that checks it.', 'A travel-time estimate can use route and traffic information. It still needs local knowledge about weather, closures, and the need to arrive early. A useful estimate is still not a promise.'], media: 'signalBoard', caption: 'Input, pattern work, output, and human review are different jobs in one practical system.' },
  ...
  { lesson: 'Lesson 3 · Learning patterns', label: 'Rules and patterns', kind: 'media', title: 'Some methods are written. Others are learned.', body: ['A clear rule can be written in advance: if a form is incomplete, return it. A learned pattern uses many examples to estimate which category or value best fits a new input.', 'A new request is not copied from one old example. The examples shape both a model’s strengths and its blind spots. That is why the example set matters so much.'], media: 'patternCards', caption: 'Varied examples gather into a pattern before a new request is considered.' },
  { lesson: 'Lesson 3 · Learning patterns', label: 'Machine-learning video', kind: 'video', title: 'Watch a pattern become a prediction', body: ['This short video makes the difference between written rules and learned patterns visible.'], video: 'machineLearning' },
  { lesson: 'Lesson 4 · Prediction engine', label: 'Next token', kind: 'media', title: 'Earlier context changes the next estimate', body: ['A language model estimates a likely next token from the words and instructions in context. It adds the token, then repeats the same process.', 'The words that came before make some continuations fit more strongly than others. Fluent output can still need checking: a likely continuation is not the same as a verified fact. This is the whole trick, repeated quickly.'], media: 'context', caption: 'The viewing window carries earlier context forward before one next piece is estimated.' },
  ...
  { lesson: 'Close', label: 'Takeaway', kind: 'summary', title: 'Use this workflow', body: ['Define the task. Name the information. Treat output as a candidate. Check important claims. Keep a person accountable for the decision. The model can help; the responsibility does not leave the room.'] }
]
    ...
    <section class="ai-player__stage" aria-live="polite">
      ...
      <article :key="screenIndex" class="ai-screen" :class="{ 'ai-screen--cover': currentScreen.kind === 'cover' }">
        ...
        <template v-else-if="currentScreen.kind === 'prediction'">
          <p class="ai-eyebrow">Prediction lab · {{ predictionIndex + 1 }} of {{ predictionRounds.length }}</p><h1 ref="screenHeading" tabindex="-1">{{ currentScreen.title }}</h1><p class="ai-lead">{{ currentScreen.body[0] }}</p><section class="prediction-card"><p class="prediction-card__context">{{ currentPrediction.context }} <b>_____</b></p><div class="choice-grid"><button v-for="(option, index) in currentPrediction.options" :key="option" type="button" :aria-pressed="predictionChoice === index" :class="{ 'is-selected': predictionChoice === index, 'is-correct': predictionChoice !== null && index === currentPrediction.correct, 'is-wrong': predictionChoice === index && !predictionCorrect }" @click="choosePrediction(index)">{{ option }}</button></div><p v-if="predictionChoice !== null" class="ai-feedback" role="status" aria-live="polite"><b>{{ predictionCorrect ? 'Good prediction.' : 'Try again.' }}</b> {{ predictionCorrect ? currentPrediction.feedback : 'Read the growing service update once more. Which choice fits the context already present?' }}</p></section>
        </template>

        <template v-else-if="currentScreen.kind === 'input'">
          <p class="ai-eyebrow">Choose the required input</p><h1 ref="screenHeading" tabindex="-1">{{ currentScreen.title }}</h1><p class="ai-lead">{{ currentScreen.body[0] }}</p><section class="choice-panel"><button type="button" :aria-pressed="inputChoice === 0" :class="{ 'is-selected': inputChoice === 0 }" @click="chooseInput(0)">Camera and sensor readings</button><button type="button" :aria-pressed="inputChoice === 1" :class="{ 'is-selected': inputChoice === 1 }" @click="chooseInput(1)">Only a long written essay</button><button type="button" :aria-pressed="inputChoice === 2" :class="{ 'is-selected': inputChoice === 2 }" @click="chooseInput(2)">A paper timetable with no location data</button><p v-if="inputChoice !== null" class="ai-feedback" role="status" aria-live="polite"><b>{{ inputChoice === 0 ? 'Correct.' : 'Look again.' }}</b> {{ inputChoice === 0 ? 'A robot needs information about position, obstacles, and movement. Cameras and sensors can describe the changing space around it.' : 'This task depends on the changing physical space. The system needs information that represents that space.' }}</p></section>
        </template>

        <template v-else-if="currentScreen.kind === 'evidence'">
          <p class="ai-eyebrow">Evidence check</p><h1 ref="screenHeading" tabindex="-1">{{ currentScreen.title }}</h1><p class="ai-lead">{{ currentScreen.body[0] }}</p><section class="choice-panel"><button type="button" :aria-pressed="evidenceChoice === 'name'" :class="{ 'is-selected': evidenceChoice === 'name' }" @click="chooseEvidence('name')">The tool has a famous model name.</button><button type="button" :aria-pressed="evidenceChoice === 'demo'" :class="{ 'is-selected': evidenceChoice === 'demo' }" @click="chooseEvidence('demo')">A short demo looked impressive once.</button><button type="button" :aria-pressed="evidenceChoice === 'test'" :class="{ 'is-selected': evidenceChoice === 'test' }" @click="chooseEvidence('test')">The team tested representative inputs, checked outcomes, and named a reviewer.</button><p v-if="evidenceChoice" class="ai-feedback" role="status" aria-live="polite"><b>{{ evidenceChoice === 'test' ? 'Correct.' : 'Not enough.' }}</b> {{ evidenceChoice === 'test' ? 'This connects a documented capability to the actual task, its conditions, and a review step.' : 'A name or one polished demo can inspire a test. It does not show reliability for this specific task.' }}</p></section>
        </template>

        <template v-else-if="currentScreen.kind === 'safeguards'">
          <p class="ai-eyebrow">Responsible-use check</p><h1 ref="screenHeading" tabindex="-1">{{ currentScreen.title }}</h1><p class="ai-lead">{{ currentScreen.body[0] }}</p><section class="safeguard-grid"><button type="button" :aria-pressed="safeguardChoices.includes('safe')" :class="{ 'is-selected': safeguardChoices.includes('safe') }" @click="toggleSafeguard('safe')">Use only safe and approved information.</button><button type="button" :aria-pressed="safeguardChoices.includes('check')" :class="{ 'is-selected': safeguardChoices.includes('check') }" @click="toggleSafeguard('check')">Check important claims against appropriate evidence.</button><button type="button" :aria-pressed="safeguardChoices.includes('review')" :class="{ 'is-selected': safeguardChoices.includes('review') }" @click="toggleSafeguard('review')">Name a person responsible for the final decision.</button><button type="button" :aria-pressed="safeguardChoices.includes('upload')" :class="{ 'is-selected': safeguardChoices.includes('upload') }" @click="toggleSafeguard('upload')">Upload personal information if the draft will be reviewed later.</button></section><p v-if="safeguardChoices.length" class="ai-feedback" role="status" aria-live="polite"><b>{{ screenReady ? 'Ready to continue.' : 'Keep checking.' }}</b> Responsible use protects information, checks consequential results, and keeps human accountability visible.</p>
        </template>
