    ...
    <section class="ai-player__stage" aria-live="polite">
      ...
      <article :key="screenIndex" class="ai-screen" :class="{ 'ai-screen--cover': currentScreen.kind === 'cover' }">
        ...
        <template v-else-if="currentScreen.kind === 'media'">
          <div class="ai-media-layout"><div><p class="ai-eyebrow">{{ currentScreen.lesson }}</p><h1 ref="screenHeading" tabindex="-1">{{ currentScreen.title }}</h1><div class="ai-prose"><p v-for="paragraph in currentScreen.body" :key="paragraph">{{ paragraph }}</p></div></div><figure class="ai-figure"><EdPaperSignal :variant="media[currentScreen.media!]" :label="currentScreen.caption" /><figcaption>{{ currentScreen.caption }}</figcaption></figure></div>
        </template>
