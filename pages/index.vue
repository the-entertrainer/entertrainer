...
const heroArtTiles = ['cobalt', 'yellow', 'violet', 'paper', 'cobalt-soft']
</script>
...
<template>
  <EdShell width="wide">
    <header class="home-hero" aria-labelledby="home-title">
      ...
      <figure class="home-hero__art home-hero__art--human u-paper-reveal">
        <img :src="homeHeroArt" alt="Editorial still life showing a practical path from tangled work to a clear route" loading="eager" decoding="async" />
        <span class="home-hero__tiles" aria-hidden="true"><i v-for="tile in heroArtTiles" :key="tile" :class="`home-hero__tile home-hero__tile--${tile}`"></i></span>
      </figure>
    </header>
...
<style scoped>
/* Handcrafted Route Atlas homepage: asymmetric route fields and quiet paper-tile movement make the art feel worked-on, not templated. */
...
.home-hero__copy p { margin: 0; color: var(--muted); font-family: var(--font-reading); font-size: clamp(18rem, 1.65vw, 22rem); line-height: 1.6; }
	.home-hero__art { position: relative; isolation: isolate; grid-column: 1 / -1; height: clamp(260rem, 34vw, 510rem); margin: clamp(12rem, 2vw, 28rem) 0 0; overflow: hidden; border: var(--stroke) solid var(--line); background: var(--paper-2); }.home-hero__art img { display: block; width: 100%; height: 100%; object-fit: cover; animation: human-drift 14s var(--ease-in-out) both; }.home-hero__tiles { position: absolute; inset: 0; z-index: 1; pointer-events: none; }.home-hero__tile { position: absolute; display: block; width: clamp(22rem, 3.5vw, 52rem); aspect-ratio: 1; border: var(--stroke) solid var(--ink); background: var(--blue); box-shadow: 5rem 5rem 0 color-mix(in srgb, var(--paper) 72%, transparent); opacity: .9; will-change: transform; animation: tile-float-a 8.6s var(--ease-in-out) infinite alternate; }.home-hero__tile::after { position: absolute; inset: 5rem; content: ''; border: 1px solid color-mix(in srgb, var(--paper) 82%, transparent); }.home-hero__tile--cobalt { top: 10%; left: 7%; transform: rotate(-7deg); }.home-hero__tile--yellow { right: 11%; bottom: 14%; background: var(--yellow); transform: rotate(8deg); animation-name: tile-float-b; animation-delay: -2.4s; }.home-hero__tile--violet { top: 18%; right: 24%; background: var(--violet); transform: rotate(4deg); animation-name: tile-float-c; animation-delay: -4.1s; }.home-hero__tile--paper { bottom: 19%; left: 28%; width: clamp(16rem, 2.3vw, 32rem); background: var(--paper); transform: rotate(12deg); animation-name: tile-float-b; animation-delay: -1.1s; }.home-hero__tile--cobalt-soft { right: 4%; top: 42%; width: clamp(15rem, 2vw, 28rem); background: color-mix(in srgb, var(--blue) 62%, var(--paper)); transform: rotate(-12deg); animation-name: tile-float-c; animation-delay: -5.5s; }
...
@keyframes home-ink { from { opacity: 0; clip-path: inset(0 100% 0 0); transform: translateY(16rem); } to { opacity: 1; clip-path: inset(0 0 0 0); transform: none; } }
	...
	@keyframes tile-float-a { from { transform: translate3d(-5rem, 8rem, 0) rotate(-7deg); } to { transform: translate3d(8rem, -6rem, 0) rotate(-3deg); } } @keyframes tile-float-b { from { transform: translate3d(7rem, -5rem, 0) rotate(8deg); } to { transform: translate3d(-7rem, 7rem, 0) rotate(13deg); } } @keyframes tile-float-c { from { transform: translate3d(5rem, 6rem, 0) rotate(4deg); } to { transform: translate3d(-6rem, -7rem, 0) rotate(-2deg); } }
