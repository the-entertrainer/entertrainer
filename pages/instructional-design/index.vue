    ...
    <section class="course-stage" aria-live="polite">
      ...
      <article :key="screenIndex" class="learning-screen screen--enter" :class="{ 'learning-screen--hero': screenIndex === 0 }">
        ...
        <template v-else-if="screenIndex === 4">
          ...
          <div class="screen-split screen-split--activity"><div class="sort-grid"><article v-for="item in sortItems" :key="item.id"><h3>{{ item.title }}</h3><p>{{ item.detail }}</p><div><button :class="{ 'is-selected': sortChoices[item.id] === 'learning' }" @click="selectSort(item.id, 'learning')">Learning can help</button><button :class="{ 'is-selected': sortChoices[item.id] === 'environment' }" @click="selectSort(item.id, 'environment')">Fix the environment</button></div></article></div><figure class="screen-visual"><img :src="media.performanceGap" alt="A café work surface showing four different causes of a performance problem: missing guidance, missing equipment, an unclear route, and lack of practice."><figcaption>Different causes need different support. Training is only one possible answer.</figcaption></figure></div><p v-if="sortComplete" class="feedback"><b>{{ sortCorrect ? 'Correct.' : 'Review your choices.' }}</b> Good analysis checks skills, tools, time, guidance, and the working environment.</p>
        </template>
